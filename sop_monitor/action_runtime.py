...
RGB 与光流权重必须由同一数据划分、同一 H3/H4 ROI 和同一采样参数训练。
本模块会从权重中读取这些元数据并校验 ROI；这条动作报警链路只向客户端上报
异常，不会修改孔位 SOP 状态机的当前步骤。
"""
...
class ActionFusionMonitor:
    ...
    ``process_frame`` 每帧都会接收画面，但只有缓存覆盖完整动作窗口且达到
    ``interval_seconds`` 时才真正推理。这样能在动作连续性和现场算力之间取平衡。
    """
