...
不直接操作 OpenCV 或海康 DLL。当前实现三条路径：本地摄像头/视频文件走普通
OpenCV，RTSP 走“后台持续读取、前台只取最新帧”，Windows 现场相机可走海康
HCNetSDK + PlayCtrl。新增取流方式时实现同一接口即可，不需要修改 SOP 状态机。

特别注意：离线视频需要保留每一帧和视频时间戳；实时监控则优先低延迟，允许
推理跟不上时丢弃旧帧。这是普通 OpenCvFrameSource 与最新帧后端分开的原因。
"""
